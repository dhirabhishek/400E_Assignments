...
#TODO - dont know what second number is so said genome; find out what the second number means
...
sambamba view -F "[XS] == null and not unmapped and not duplicate" -c align.sorted.bam
...
#?# What happened? Enter your answer below - 1 pt
## Below is what it is printing when the above command is ran. So it looks like it was performing fastqc on the files we wanted it to run on. And then it started printing outputs that are printed when running fastqc on a file.
## running pipeline for iPSC_input
##Performing fastqc of sample iPSC_input with the following fastqs:
##/projects/bmeg/A2/input_iPSC_SRA66_subset_1.fastq.gz  ##/projects/bmeg/A2/input_iPSC_SRA66_subset_2.fastq.gz
